...
class ColorsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create color" do
    assert_difference("Color.count") do
      post colors_url, params: { color: { description: @color.description, product_id: @color.product_id, rgb: @color.rgb } }
    end
  ...
  test "should update color" do
    patch color_url(@color), params: { color: { description: @color.description, product_id: @color.product_id, rgb: @color.rgb } }
