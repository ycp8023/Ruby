...
class SizesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create size" do
    assert_difference("Size.count") do
      post sizes_url, params: { size: { description: @size.description, product_id: @size.product_id, size_name: @size.size_name } }
    end
  ...
  test "should update size" do
    patch size_url(@size), params: { size: { description: @size.description, product_id: @size.product_id, size_name: @size.size_name } }
